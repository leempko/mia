...
%
In 2D, there will therefore by two sets of parameters: $\fat{w} = (\fat{w}_1\transp, \fat{w}_2\transp)\transp$,
whereas in 3D there will be three: $\fat{w} = (\fat{w}_1\transp, \fat{w}_2\transp, \fat{w}_3\transp )\transp$.
...
+ 
\sum_{d=1}^D \sum_{m=0}^{M-1} \big( g_{d,n} \phi_m( \fat{x}_n ) \big) \epsilon_{d,m}
.
\label{eq:taylor}
\end{equation}
...
Maximizing the likelihood function $p( \fat{t} | \fat{X}, \bldgr{\theta} )$ is equivalent to minimizing its negative logarithm. Using \eqref{eq:Bernoulli} and \eqref{eq:NNLikelihood}, training therefore consists of minimizing the energy function 
\begin{equation}
  E_N( \bldgr{\theta} ) 
